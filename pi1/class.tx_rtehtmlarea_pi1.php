	...
	class tx_rtehtmlarea_pi1 extends tslib_pibase {
		...
		function main($conf) {
			...
			if(!$safe_mode_is_enabled && (!$this->pspell_is_available || $this->forceCommandMode)) {
				...
				if( doubleval($AspellVersion) < doubleval('0.5') && (!$this->pspell_is_available || $this->forceCommandMode)) echo('Configuration problem: Aspell version too old. Spell checking cannot be performed in command mode');
			}
			...
			while( list(,$word) = each($words) ) {
				...
				if( $word && !is_numeric($word)) {
					if($this->pspell_is_available && !$this->forceCommandMode) {
						...
						$AspellCommand = 'cat ' . $tmpFileName . ' | ' . $this->AspellDirectory . ' -a check --mode=none --sug-mode=' . $this->pspellMode . ' --lang=' .$this->dictionary . ' --encoding=' . $this->parserCharset . ' 2>&1';
