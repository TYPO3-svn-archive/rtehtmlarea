 ...
 * @subpackage core
 */
class tx_rtehtmlarea_TBE_browser_recordList extends localRecordList {
 ...
 * @subpackage core
 */
class tx_rtehtmlarea_localPageTree extends t3lib_browseTree {
	 ...
	 * @return	void
	 */
	function tx_rtehtmlarea_localPageTree() {
		$this->init();
 ...
 * @subpackage core
 */
class tx_rtehtmlarea_rtePageTree extends tx_rtehtmlarea_localPageTree {
}
...
class tx_rtehtmlarea_TBE_PageTree extends  tx_rtehtmlarea_localPageTree {
 ...
 * @subpackage core
 */
class tx_rtehtmlarea_links_localFolderTree extends t3lib_folderTree {
	var $ext_IconMode=1;
	...
	function wrapTitle($title,$v)	{
		if ($this->ext_isLinkable($v))	{
			$aOnClick = 'return jumpToUrl(\''.$this->script.'?act='.$GLOBALS['SOBE']->act.'&mode='.$GLOBALS['SOBE']->mode.'&expandFolder='.rawurlencode($v['path']).'\');';
			return '<a href="#" onclick="'.htmlspecialchars($aOnClick).'">'.$title.'</a>';
		} else {
 ...
 * @subpackage core
 */
class tx_rtehtmlarea_browse_links {
	...
	function init()	{
		...
		if ((string)$this->mode=='rte')	{
			...
			if($this->thisConfig['classesAnchor']) {
				...
				$selected = '';
				if(!$this->setClass)  $selected = 'selected';
				$this->classesAnchorJSOptions = '<option ' . $selected . ' value=""></option>';
				reset($classesAnchorArray);
				while(list(,$class)=each($classesAnchorArray)) {
					$selected = '';
					if($this->setClass == $class) $selected = 'selected';
					$this->classesAnchorJSOptions .= '<option ' . $selected . ' value="' .$class . '">' . $class . '</option>';
				}
			}
		}
...
';
	...
	function main_rte($wiz=0)	{
		...
		$menu='
			...
			<!--
				RTE Link top menu:
			-->
			<table border="0" cellpadding="2" cellspacing="1" id="typo3-RTElinkMenu">
				<tr>
					';
		...
		if (in_array('file',$allowedItems))	$menu.='
					...
					<td'.($this->act=='file'?$bgcolorA:$bgcolor).'><a href="#" onclick="jumpToUrl(\'?act=file\');return false;">'.$GLOBALS['LANG']->getLL('file',1).'</a></td>';
		if (in_array('url',$allowedItems))	$menu.='
					<td'.($this->act=='url'?$bgcolorA:$bgcolor).'><a href="#" onclick="jumpToUrl(\'?act=url\');return false;">'.$GLOBALS['LANG']->getLL('extUrl',1).'</a></td>';
		if (in_array('mail',$allowedItems))	$menu.='
		...
		switch($this->act)	{
			...
			break;
			case 'file':
				$foldertree = t3lib_div::makeInstance('tx_rtehtmlarea_links_rteFolderTree');
			...
			case 'page':
			default:
				$pagetree = t3lib_div::makeInstance('tx_rtehtmlarea_rtePageTree');
		...
		if ($this->act!='mail')	{
			...
			$ltarget.='		</td>
				...
				// Class:
			if($this->classesAnchorJSOptions) {
				$selectClassJS = '
			...
			$content.=$ltarget;
				...
				// Add some space
			$content.='<br /><br />';
		} elseif ($this->classesAnchorJSOptions) {
			$selectClassJS = '
				document.ltargetform.anchor_class.value = document.ltargetform.anchor_class.options[document.ltargetform.anchor_class.selectedIndex].value;
				setClass(document.ltargetform.anchor_class.value);
			';
			...
			-->
				...
				// Add "class selector" box to content:
			$content.=$ltarget;
	...
	function main_db()	{
		...
		$pagetree = t3lib_div::makeInstance('tx_rtehtmlarea_TBE_PageTree');
		$pagetree->script='rtehtmlarea_browse_links.php';
	...
	function main_file()	{
		...
		if (!$noThumbs)	{
			...
			// Create folder tree:
		$foldertree = t3lib_div::makeInstance('tx_rtehtmlarea_TBE_FolderTree');
		$foldertree->script='rtehtmlarea_browse_links.php';
	...
	function TBE_expandPage($tables)	{
		...
		if ($this->expandPage>=0 && t3lib_div::testInt($this->expandPage) && $BE_USER->isInWebMount($this->expandPage))	{
			...
			$dblist = t3lib_div::makeInstance('tx_rtehtmlarea_TBE_browser_recordList');
...
// Make instance:
$SOBE = t3lib_div::makeInstance('tx_rtehtmlarea_browse_links');
$SOBE->init();
$SOBE->main();
$SOBE->printContent();
?>